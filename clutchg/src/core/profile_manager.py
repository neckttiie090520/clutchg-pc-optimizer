...
class ProfileManager:
    ...
    def apply_profile(self, 
        ...
        logger.info(f"Profile application complete: {successful} successful, {failed} failed")
    ...
    def apply_tweaks(
        ...
        missing_scripts = [s for s in script_groups if not (self.scripts_dir / s).exists()]
        if missing_scripts:
            error_msg = f"Missing scripts: {', '.join(missing_scripts)}"
            logger.error(f"Pre-validation failed: {error_msg}")
            if on_output:
                on_output(f"❌ Pre-validation failed — {len(missing_scripts)} script(s) not found:")
                for ms in missing_scripts:
                    on_output(f"   • {ms}")
                on_output("")
                on_output("Aborting. No changes were made to the system.")
            return ExecutionResult(
                success=False, output='', errors=error_msg,
        ...
        if on_output:
            on_output("")
            on_output(f"{'✅' if overall_success else '⚠️'} "
    ...
    def save_custom_preset(self, name: str, tweak_ids: List[str]) -> bool:
        ...
        config_path = Path(__file__).parent.parent / "config" / "custom_presets.json"
    ...
    def export_preset_to_file(self, name: str, tweak_ids: List[str], filepath: Path) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Exported preset '{name}' ({len(tweak_ids)} tweaks) to {filepath}")
            return True
        except Exception as e:
