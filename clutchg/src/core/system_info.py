...
class CPUInfo:
    ...
    score: int  # 0-30
    benchmark_matched: bool = False  # True when matched in benchmark DB
...
class SystemDetector:
    ...
    def detect_all(self) -> SystemProfile:
        ...
        # Calculate scores
        total_score = (
            cpu_info.score + gpu_info.score + ram_info.score + storage_info.score
        )
    ...
    def detect_os(self) -> OSInfo:
        ...
        if "windows" in system:
            ...
            build = (
                platform.win32_ver()[1] if hasattr(platform, "win32_ver") else "unknown"
            )
        else:
    ...
    def detect_cpu(self) -> CPUInfo:
        ...
        score = cpu_score
        cpu_matched = matched_name != "Unknown CPU"
    ...
    def detect_gpu(self) -> GPUInfo:
        ...
        if name == "Unknown GPU" and self.wmi_conn:
            try:
                for gpu in self.wmi_conn.Win32_VideoController():
                    ...
                    is_discrete = any(
                        x in gpu_name.lower()
                        for x in ["nvidia", "geforce", "rtx", "gtx", "radeon"]
                    )
        ...
        # Calculate score using benchmark database
        gpu_score, raw_score, gpu_vram_db, matched_name = (
            self.benchmark_db.get_gpu_score(name)
        )
        score = gpu_score
        gpu_matched = matched_name != "Unknown GPU"
    ...
    def detect_ram(self) -> RAMInfo:
        ...
        return RAMInfo(total_gb=total_gb, type=ram_type, speed=speed, score=score)
    ...
    def detect_storage(self) -> StorageInfo:
        ...
        score = (
            10
            if primary_type in ["nvme", "ssd"]
            else (5 if primary_type == "hdd" else 3)
        )

        return StorageInfo(primary_type=primary_type, total_gb=total_gb, score=score)
    ...
    def recommend_profile(self, system: SystemProfile) -> str:
        ...
        .. deprecated::
            ...
            to the unified recommendation service.
        ...
        result = recommend_preset(system)
        return result.preset.upper()
