...
from core.paths import snapshot_dir
...
class SystemSnapshot:
    ...
    power_plan: str = ""
    visual_effects_level: str = (
        ""  # "best_appearance" | "balanced" | "best_performance"
    )
...
class SnapshotDiff:
    ...
    def summary_lines(self) -> List[str]:
        ...
        if self.tasks_disabled_delta < 0:
            lines.append(
                f"📋 {abs(self.tasks_disabled_delta)} scheduled task(s) disabled"
            )

        if self.power_plan_changed:
            lines.append(
                f"🔋 Power plan: {self.power_plan_before} → {self.power_plan_after}"
            )
...
class SystemSnapshotManager:
    ...
    def take_snapshot(self) -> SystemSnapshot:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Unexpected PowerShell output for running services: {result.stdout}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Unexpected PowerShell output for stopped services: {result2.stdout}"
                )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Unexpected PowerShell output for startup items: {result.stdout}"
                )
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Unexpected PowerShell output for scheduled tasks: {result.stdout}"
                )
        except Exception as e:
        ...
        logger.info(
            f"Snapshot taken: {snap.services_running} running, "
    ...
    def compare(self, before: SystemSnapshot, after: SystemSnapshot) -> SnapshotDiff:
        ...
        return SnapshotDiff(
            ...
            tasks_disabled_delta=after.scheduled_tasks_enabled
            - before.scheduled_tasks_enabled,
