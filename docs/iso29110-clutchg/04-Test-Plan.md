...
| กิจกรรม | ตรวจอะไร | เทียบกับอะไร | วิธี |
|---------|---------|-------------|-----|
| Unit Testing | Individual modules | Expected behavior | pytest (939 cases) |
...
|---|-------|----------|---------|------------|----------|
| 1 | **Unit** | Function/method เดี่ยว | Yes | 939 | pytest + mock |
...
Measured with `python -m pytest tests/unit tests/integration --cov=src/core` from `clutchg`; 962 tests passed.
...
| Unit Test Pass Rate | ≥ 95% | 100% (939/939) | EXCEEDS |
...
- **ผลลัพธ์:** 939 unit + 23 integration test cases, pass rate 100% (measured 2026-08-05)
