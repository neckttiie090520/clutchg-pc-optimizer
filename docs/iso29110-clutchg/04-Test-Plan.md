...
| กิจกรรม | ตรวจอะไร | เทียบกับอะไร | วิธี |
|---------|---------|-------------|-----|
| Unit Testing | Individual modules | Expected behavior | pytest (931 cases) |
...
|---|-------|----------|---------|------------|----------|
| 1 | **Unit** | Function/method เดี่ยว | Yes | 931 | pytest + mock |
...
Measured with `python -m pytest tests/unit tests/integration --cov=src/core` from `clutchg`; 954 tests passed.
...
| Unit Test Pass Rate | ≥ 95% | 100% (931/931) | EXCEEDS |
...
- **ผลลัพธ์:** 931 unit + 23 integration test cases, pass rate 100% (measured 2026-08-05)
